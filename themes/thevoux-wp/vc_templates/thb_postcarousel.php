<?php function thb_postcarousel( $atts, $content = null ) {
  ...
  extract( $atts );
	...
	if ( $posts->have_posts() ) { ?>
		<div class="slick <?php echo esc_attr($style_class); ?><?php if($pagi == 'true') { ?> dark-pagination bottom-margin<?php } ?> <?php echo ($style == 'style3' && $nav == 'true') ? 'outset-nav' : ''; ?> <?php echo ($style == 'style3') ? 'mini-columns' : ''; ?>" data-center="<?php echo ($style == 'style3' || $style == 'style4') ? 'false' : 'true'; ?>" data-columns="<?php echo $columns; ?>" data-pagination="<?php echo $pagi; ?>" data-navigation="<?php echo $nav; ?>">
					...
					<article <?php post_class('post featured-style4'); ?>>
						...
						<div class="featured-title">
							...
							<div class="post-excerpt">
								<?php echo thb_excerpt(80); ?>
								<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>" class="more"><?php _e('Read More &gt;', 'thevoux' ); ?></a>
							</div>
